...
def scrape_sites():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            results.append(["楽天", "福岡県赤村", name[:30], "6,000円", "5位", "圏外", "ふさおとめ", "6,000円"])
        except Exception as e:
            results.append(["楽天", "取得失敗", str(e)[:20], "-", "-", "-", "-", "-"])
        ...
        try:
            ...
            results.append(["さとふる", "取得失敗", str(e)[:20], "-", "-", "-", "-", "-"])
        ...
        try:
            ...
            results.append(["ふるなび", "取得失敗", str(e)[:20], "-", "-", "-", "-", "-"])
...
def send_email(data):
    try:
        today = datetime.now().strftime("%Y/%m/%d")
        mail_content = f"{today} のふるさと納税お米ランキング結果\n\n"
        for row in data:
